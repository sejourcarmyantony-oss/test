...
const { updateUserConfig } = require('../lib/githubDB');
...
// ============================================================
// 4bis / 5bis. Liens groupe & channel supplémentaires (2 et 3)
// Permettent à l'autojoin (lib/autojoin.js) de rejoindre jusqu'à
// 3 groupes et suivre jusqu'à 3 channels à la connexion.
// ============================================================
cmd({
    pattern: "setgrouplink2",
    desc: "Changer le 2ème lien de groupe WhatsApp (autojoin)",
    category: "settings",
    react: "👥"
},
async (conn, mek, m, { args, isOwner, reply, botNumber, config }) => {
    ...
    const newLink = args[0];
    if (!newLink) {
        return reply(`*2ème lien groupe actuel :❯ ${config.GROUP_INVITE_LINK_2 || 'aucun'}*\n\nPour changer, écris :\n*.setgrouplink2 https://chat.whatsapp.com/xxxx*`);
    }
    if (!newLink.startsWith('https://chat.whatsapp.com/')) {
        return reply("❌ Lien invalide. Le lien doit commencer par https://chat.whatsapp.com/");
    }
    await updateConfig('GROUP_INVITE_LINK_2', newLink, botNumber, config, reply);
});

cmd({
    pattern: "setgrouplink3",
    desc: "Changer le 3ème lien de groupe WhatsApp (autojoin)",
    category: "settings",
    react: "👥"
},
async (conn, mek, m, { args, isOwner, reply, botNumber, config }) => {
    ...
    const newLink = args[0];
    if (!newLink) {
        return reply(`*3ème lien groupe actuel :❯ ${config.GROUP_INVITE_LINK_3 || 'aucun'}*\n\nPour changer, écris :\n*.setgrouplink3 https://chat.whatsapp.com/xxxx*`);
    }
    if (!newLink.startsWith('https://chat.whatsapp.com/')) {
        return reply("❌ Lien invalide. Le lien doit commencer par https://chat.whatsapp.com/");
    }
    await updateConfig('GROUP_INVITE_LINK_3', newLink, botNumber, config, reply);
});

cmd({
    pattern: "setchannellink2",
    desc: "Changer le 2ème channel WhatsApp (autojoin) — lien ou JID xxx@newsletter",
    category: "settings",
    react: "📢"
},
async (conn, mek, m, { args, isOwner, reply, botNumber, config }) => {
    ...
    const newLink = args[0];
    if (!newLink) {
        return reply(`*2ème lien channel actuel :❯ ${config.CHANNEL_LINK_2 || 'aucun'}*\n\nPour changer, écris :\n*.setchannellink2 https://whatsapp.com/channel/xxxx* (ou directement xxxx@newsletter)`);
    }
    if (!newLink.startsWith('https://whatsapp.com/channel/') && !newLink.endsWith('@newsletter')) {
        return reply("❌ Lien invalide. Le lien doit commencer par https://whatsapp.com/channel/ ou être un JID xxxx@newsletter");
    }
    await updateConfig('CHANNEL_LINK_2', newLink, botNumber, config, reply);
});

cmd({
    pattern: "setchannellink3",
    desc: "Changer le 3ème channel WhatsApp (autojoin) — lien ou JID xxx@newsletter",
    category: "settings",
    react: "📢"
},
async (conn, mek, m, { args, isOwner, reply, botNumber, config }) => {
    ...
    const newLink = args[0];
    if (!newLink) {
        return reply(`*3ème lien channel actuel :❯ ${config.CHANNEL_LINK_3 || 'aucun'}*\n\nPour changer, écris :\n*.setchannellink3 https://whatsapp.com/channel/xxxx* (ou directement xxxx@newsletter)`);
    }
    if (!newLink.startsWith('https://whatsapp.com/channel/') && !newLink.endsWith('@newsletter')) {
        return reply("❌ Lien invalide. Le lien doit commencer par https://whatsapp.com/channel/ ou être un JID xxxx@newsletter");
    }
    await updateConfig('CHANNEL_LINK_3', newLink, botNumber, config, reply);
});
...
📞 *Numéro(s) dev* : ${owners}
📢 *Channel 1* : ${config.CHANNEL_LINK}
📢 *Channel 2* : ${config.CHANNEL_LINK_2 || 'aucun'}
📢 *Channel 3* : ${config.CHANNEL_LINK_3 || 'aucun'}
👥 *Groupe 1* : ${config.GROUP_INVITE_LINK}
👥 *Groupe 2* : ${config.GROUP_INVITE_LINK_2 || 'aucun'}
👥 *Groupe 3* : ${config.GROUP_INVITE_LINK_3 || 'aucun'}
