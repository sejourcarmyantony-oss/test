...
function arslanLog(message, type = 'info') {
    ...
    console.log(`${icons[type] || '📝'} [Olsen-MD-MINI] ${new Date().toISOString()}: ${message}`);
}
    ...
    try {
        ...
        const conn = makeWASocket({
            ...
            getMessage: async (key) => {
                ...
                return msg && msg.message ? msg.message : { conversation: 'Olsen-MD' };
            }
        });
    ...
    try {
        ...
        conn.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'open') {
                ...
                if (!existingSession) {
                    ...
                    const welcomeBotName = welcomeConfig.BOT_NAME || 'Olsen-MD';
        ...
        conn.ev.on('messages.upsert', async (msg) => {
            try {
                ...
                const displayBotName = effectiveConfig.BOT_NAME || 'Olsen-MD';
                const myquoted = {
        ...
        arslanLog(`Olsen-MD-MINI Pair error: ${err.message}`, 'error');
...
router.get('/update-config', async (req, res) => {
    ...
    try {
        await socket.sendMessage(jidNormalizedUser(socket.user.id), { text: `*🔐 Olsen-MD — CONFIG UPDATE*\n\nOTP: *${otp}*\nValid 5 minutes` });
